...
export default function POSPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (e.error === "INSUFFICIENT_STOCK") {
        setCheckoutError(
          isRTL
            ? `الكمية مش كفاية: ${e.productName} (المتاح: ${e.available})`
            : `Insufficient stock: ${e.productName} (available: ${e.available})`
        );
      } else if (e.error === "EXPIRED_BATCH") {
        setCheckoutError(
          isRTL
            ? "فيه دفعة منتهية في السلة، شيل الصنف وضيفه تاني."
            : "Expired batch in cart — please remove and re-add the item"
        );
      } else {
        setCheckoutError(
          isRTL ? "إتمام البيع فشل، حاول مرة تانية." : "Checkout failed — please try again"
        );
      }
    } finally {
  ...
  const t = {
    ...
    amountPaid: isRTL ? "المبلغ المستلم" : "Amount Paid",
  };
    ...
    <div dir={isRTL ? "rtl" : "ltr"} className="space-y-6">
      <div className="rounded-[var(--radius-xl)] border border-[color:color-mix(in_srgb,var(--accent)_18%,transparent)] bg-[linear-gradient(135deg,color-mix(in_srgb,var(--accent)_10%,var(--card)_90%),color-mix(in_srgb,var(--surface)_86%,transparent))] p-6 shadow-[0_16px_40px_rgba(15,23,42,0.05)]">
        ...
        <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[var(--accent)]">
          {isRTL ? "شاشة الكاشير" : "Front desk flow"}
        </p>
        ...
        <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
          {isRTL
            ? "امسح الباركود أو دوّر بسرعة، وراجع السلة واقفل البيعة من نفس الشاشة."
            : "Search products, review the basket, and close the sale from one calmer workspace."}
        </p>
      </div>
              ...
              {searchResults.map((product) => (
                ...
                >
                  ...
                  <div className="text-right rtl:text-left ml-4 rtl:mr-4 rtl:ml-0 flex-shrink-0">
                    ...
                    <p
                      className={`text-xs ${product.currentQuantity < 10 ? "text-orange-500" : "text-gray-400"}`}
                    >
          ...
          {!searchQuery && cart.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                {isRTL
                  ? "دوّر على صنف علشان تضيفه للسلة"
