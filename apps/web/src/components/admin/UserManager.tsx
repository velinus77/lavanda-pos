...
const translations: Record<"ar" | "en", Translations> = {
  ...
  ar: {
    ...
    activate: "تفعيل",
    searchPlaceholder: "دوّر بالاسم أو اسم المستخدم أو الإيميل...",
    showing: "عرض",
    of: "من",
    users: "مستخدمين",
    previous: "السابق",
    ...
    active: "نشط",
    inactive: "غير نشط",
    confirmDeactivate: "متأكد إنك عايز توقف المستخدم ده؟",
    confirmActivate: "متأكد إنك عايز تفعّل المستخدم ده؟",
    loading: "جارٍ التحميل...",
    error: "حصل خطأ",
    successCreate: "المستخدم اتضاف بنجاح",
    successUpdate: "بيانات المستخدم اتحدثت بنجاح",
    successDelete: "المستخدم اتوقف بنجاح",
    passwordsMismatch: "كلمتا المرور مش زي بعض",
    requiredField: "الخانة دي مطلوبة",
    leaveBlankToKeep: "اتركه فارغًا للاحتفاظ بكلمة المرور الحالية",
    invalidEmail: "الإيميل مش بصيغة صحيحة",
    shortPassword: "كلمة المرور لازم تكون 8 حروف على الأقل",
    noUsers: "مفيش مستخدمين",
    authRequired: "لازم تسجل دخول الأول",
  },
};
...
function inputClass(theme: "light" | "dark", hasError = false) {
  return `w-full rounded-[var(--radius-md)] border px-4 py-3 outline-none transition-all focus:border-[var(--action)] focus:ring-2 focus:ring-[color:var(--action)]/15 ${
    theme === "dark"
  ...
  } ${hasError ? "border-[var(--danger)] focus:border-[var(--danger)] focus:ring-[color:var(--danger)]/15" : ""}`;
}
  ...
  return (
    <div className="mt-6 flex justify-end gap-3">
      <button
        ...
        className={
          theme === "dark"
            ? "rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-4 py-2.5 font-medium text-[var(--muted)] transition-colors hover:bg-[var(--surface-strong)]"
            : "rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-4 py-2.5 font-medium text-[var(--foreground)] transition-colors hover:bg-[var(--surface-strong)]"
        }
      >
        {cancelLabel}
      </button>
      <button
        onClick={onConfirm}
        className="rounded-[var(--radius-md)] bg-[var(--action)] px-4 py-2.5 font-medium text-white shadow-[0_14px_28px_rgba(31,157,115,0.22)] transition-colors hover:bg-[var(--action-strong)]"
      >
        {actionLabel}
      </button>
    </div>
    ...
    <div className="space-y-6" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[color:color-mix(in_srgb,var(--surface)_76%,transparent)] p-4">
        <div className="relative">
          ...
          />
          <svg
            className={`absolute top-1/2 h-5 w-5 -translate-y-1/2 text-[var(--muted)] ${isRTL ? "left-3" : "right-3"}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
                  ...
                  {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((user) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${user.role === "admin" ? "bg-[var(--warning-soft)] text-[var(--warning)]" : user.role === "manager" ? "bg-[var(--info-soft)] text-[var(--info)]" : "bg-[var(--action-soft)] text-[var(--action)]"}`}
                          >
                            {roleLabels[locale][user.role]}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${user.is_active ? "bg-[var(--action-soft)] text-[var(--action)]" : "bg-[var(--danger-soft)] text-[var(--danger)]"}`}
                          >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              className={`font-medium ${user.is_active ? "text-[var(--danger)] hover:opacity-80" : "text-[var(--action)] hover:opacity-80"}`}
                            >
            ...
            {totalPages > 1 ? (
              <div className="flex items-center justify-between border-t border-[var(--border)] px-6 py-4">
                <p className="text-sm text-[var(--muted)]">
                  {t.showing} {Math.min((currentPage - 1) * usersPerPage + 1, totalUsers)} -{" "}
                  {Math.min(currentPage * usersPerPage, totalUsers)} {t.of} {totalUsers} {t.users}
                </p>
                <div className="flex gap-2">
                  <button
                    onClick={() => setCurrentPage((page) => Math.max(1, page - 1))}
                    disabled={currentPage === 1}
                    className={`rounded-[var(--radius-md)] px-3 py-1.5 text-sm font-medium transition-colors ${currentPage === 1 ? "cursor-not-allowed bg-[var(--surface)] text-[var(--muted)]" : "bg-[var(--action)] text-white hover:bg-[var(--action-strong)]"}`}
                  >
                    {t.previous}
                  </button>
                  <button
                    ...
                    className={`rounded-[var(--radius-md)] px-3 py-1.5 text-sm font-medium transition-colors ${currentPage === totalPages ? "cursor-not-allowed bg-[var(--surface)] text-[var(--muted)]" : "bg-[var(--action)] text-white hover:bg-[var(--action-strong)]"}`}
                  >
                    {t.next}
                  </button>
                </div>
      ...
      >
        <div className="space-y-4">
          <FormField label={t.username} error={formErrors.username}>
            <input
              type="text"
              value={formData.username}
              onChange={(e) => setFormData({ ...formData, username: e.target.value })}
              placeholder={t.usernamePlaceholder}
              className={inputClass(theme, !!formErrors.username)}
            />
          </FormField>
          <FormField label={t.fullName} error={formErrors.full_name}>
            <input
              type="text"
              value={formData.full_name}
              onChange={(e) => setFormData({ ...formData, full_name: e.target.value })}
              placeholder={t.fullNamePlaceholder}
              className={inputClass(theme, !!formErrors.full_name)}
            />
          </FormField>
          <FormField label={t.emailLabel} error={formErrors.email}>
            <input
              type="email"
              value={formData.email}
              onChange={(e) => setFormData({ ...formData, email: e.target.value })}
              placeholder={t.emailPlaceholder}
              className={inputClass(theme, !!formErrors.email)}
            />
          </FormField>
          <FormField label={t.password} error={formErrors.password}>
            <input
              ...
              className={inputClass(theme, !!formErrors.password)}
            />
          </FormField>
          <FormField label={t.confirmPassword} error={formErrors.confirmPassword}>
            <input
              type="password"
              value={confirmPassword}
              onChange={(e) => setConfirmPassword(e.target.value)}
              placeholder={t.confirmPasswordPlaceholder}
              className={inputClass(theme, !!formErrors.confirmPassword)}
            />
          </FormField>
          <FormField label={t.selectRole}>
            <select
              value={formData.role}
        ...
        <ModalActions
          ...
          actionLabel={submitting ? t.loading : t.create}
          onCancel={() => {
            setIsAddModalOpen(false);
            resetForm();
          }}
      ...
      >
        <div className="space-y-4">
          ...
          <FormField label={`${t.password} (${t.leaveBlankToKeep})`} error={formErrors.password}>
            <input
              type="password"
              value={formData.password}
              onChange={(e) => setFormData({ ...formData, password: e.target.value })}
        ...
        <ModalActions
          theme={theme}
          cancelLabel={t.cancel}
          actionLabel={submitting ? t.loading : t.update}
          onCancel={() => {
            setIsEditModalOpen(false);
            setSelectedUser(null);
            resetForm();
      ...
      >
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            className={`rounded-[var(--radius-md)] px-4 py-2.5 font-medium text-white transition-colors ${selectedUser?.is_active ? "bg-[var(--danger)] hover:opacity-90" : "bg-[var(--action)] hover:bg-[var(--action-strong)]"}`}
          >
